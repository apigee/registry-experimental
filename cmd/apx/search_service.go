...
func init() {
	...
	SearchConfig.SetEnvPrefix("APX_SEARCH")
	SearchConfig.AutomaticEnv()

	SearchServiceCmd.PersistentFlags().Bool("insecure", false, "Make insecure client connection. Or use APX_SEARCH_INSECURE. Must be used with \"address\" option")
	SearchConfig.BindPFlag("insecure", SearchServiceCmd.PersistentFlags().Lookup("insecure"))
	SearchConfig.BindEnv("insecure")

	SearchServiceCmd.PersistentFlags().String("address", "", "Set API address used by client. Or use APX_SEARCH_ADDRESS.")
	SearchConfig.BindPFlag("address", SearchServiceCmd.PersistentFlags().Lookup("address"))
	SearchConfig.BindEnv("address")

	SearchServiceCmd.PersistentFlags().String("token", "", "Set Bearer token used by the client. Or use APX_SEARCH_TOKEN.")
	SearchConfig.BindPFlag("token", SearchServiceCmd.PersistentFlags().Lookup("token"))
	SearchConfig.BindEnv("token")

	SearchServiceCmd.PersistentFlags().String("api_key", "", "Set API Key used by the client. Or use APX_SEARCH_API_KEY.")
