...
if [[ $deployment ]]; then
    ...
    envsubst < deployments/worker-setup/worker/worker-deployment.yaml | kubectl apply -f -
fi
...
# Worker service
kubectl apply -f deployments/worker-setup/worker/worker-service.yaml
...
if [[ $deployment ]]; then
    ...
    envsubst < deployments/worker-setup/dispatcher/dispatcher-deployment.yaml | kubectl apply -f -
fi
